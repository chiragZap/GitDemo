...
@property (nonatomic, assign) JGScrollableTableViewCell <JGTouchForwarder, JGViewProvider> *parentCell;
...
+ (void)OPenAllCellsWithExceptionOf:(JGScrollableTableViewCell *)cell stopAfterFirst:(BOOL)stop {
    UITableView *host = (UITableView *)cell.superview;
    NSSet *cells = [self allCellsInTableView:host];
    
    for (JGScrollableTableViewCell *otherCell in cells) {
        if (otherCell != cell && (otherCell.scrolling || otherCell.optionViewVisible)) {
            [otherCell setOptionViewVisible:YES animated:YES];
            if (stop) {
                break;
            }
        }
